  ...
  public levelOne(req, res, next) {
    if (req.name && req.rol) {
      ...
      if (level === 1) {
        ...
        return res
          .status(405)
          .send({ auth: false, message: 'This user is not allowed.' });
      }
    }
  }
  /**
    ...
    role.findOneAndUpdate(
      ...
      (err, user) => {
        if (err) {
          res
            .status(404)
            .send({ error: { code: '404', message: 'user not found' } });
        }
        let passwordIsValid =
        ...
        if (!passwordIsValid)
          return res
            .status(401)
            .send({ error: { code: '403', message: 'Invalid password' } });
        // create a token
  ...
  public getCoaches(req, res) {
    role.find({ rol: 'coach' }, (err, coches) => {
      if (err) {
        res
          .status(404)
          .send({ error: { code: '404', message: 'coaches not found' } });
      }
  ...
  public getManagers(req, res) {
    role.find({ rol: 'manager' }, (err, managers) => {
      if (err) {
        res
          .status(404)
          .send({ error: { code: '404', message: 'manager not found' } });
      }
  ...
  public getCapturist(req, res) {
    role.find({ rol: 'capturist' }, (err, capturists) => {
      if (err) {
        res
          .status(404)
          .send({ error: { code: '404', message: 'capturist not found' } });
      }
